...
class HyphenationRules
        ...
        if all_letters?(inflection)
            ...
            if inflection.include? "+%"
                ...
                if split_length = inflection.reverse.index('%')
                    log = log + "\n plusprosent-split: #{word[-split_length..-1]}"
    ...
    def rule_of_one_consonant(word, hyphenations, length, log)
        ...
        word.each_char_with_index do |current_letter, i|
            ...
            # Check if seen: One vowel and consonant in prefix, current letter is consonant and prefix starts on a vowel
            if include_vowel?(prefix) && include_consonant?(prefix) && include_consonant?(current_letter) && include_vowel?(suffix[0])
